...
#import "MEMultiplePulsingHaloLayer.h"
#import "MEPulsingHaloLayer.h"
#import "MEConstants.h"

@implementation MEMultiplePulsingHaloLayer
...
- (void)setHaloLayerColor:(CGColorRef)backgroundColor {
    ...
    for (MEPulsingHaloLayer *layer in self.sublayers) {
...
- (void)buildSublayers {
    for (int i = 0; i < self.haloLayerNumber; i++) {
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(self.startInterval* i * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            MEPulsingHaloLayer *layer = [MEPulsingHaloLayer layer];
    ...
    DDLogDebug(@"sub count: %tu", self.sublayers.count);
}
